...
Windows icon and version resources are generated by the build tool, so `go-winres` does not need to be installed manually. The first `go run ./tools/build` run downloads `github.com/tc-hib/go-winres` automatically. WebView2 builds also require the Microsoft Edge WebView2 Runtime on the machine that runs the executable; it is usually preinstalled on Windows 10/11.
